...
export function AudioReader({ text }: AudioReaderProps) {
  ...
  const playBrowserTTS = () => {
    if (!("speechSynthesis" in window)) {
      alert("お使いのブラウザはブラウザ音声読み上げに対応していません。");
      return;
    }

    const utterance = new SpeechSynthesisUtterance(text);
    utterance.lang = /[ぁ-んァ-ン一-龠]/.test(text) ? "ja-JP" : "en-US";
      ...
      if (creditsHeader) {
        window.dispatchEvent(
          new CustomEvent("credits_updated", {
            detail: { credits: parseInt(creditsHeader, 10) },
          }),
        );
      }
  ...
  return (
    <div className="flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 p-4 sm:p-5">
      <div className="flex flex-wrap items-center gap-3">
        <div className="mb-1 flex w-full items-center gap-2 font-medium text-neutral-300 sm:mb-0 sm:w-auto">
        ...
        <button
          ...
          className="relative flex w-full items-center overflow-hidden rounded-full border border-white/10 bg-neutral-950 p-1 sm:w-fit"
        >
          <div
            className={`relative z-10 flex flex-1 items-center justify-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold whitespace-nowrap transition-colors sm:flex-none ${
              engine === "browser" ? "text-white" : "text-neutral-500 hover:text-neutral-300"
            }`}
          >
          ...
          <div
            className={`relative z-10 flex flex-1 items-center justify-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold whitespace-nowrap transition-colors sm:flex-none ${
              engine === "ai" ? "text-white" : "text-neutral-500 hover:text-neutral-300"
            }`}
          >
